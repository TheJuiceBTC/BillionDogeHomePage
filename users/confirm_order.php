...
?>
	...
	if (($has_packages) && ($_REQUEST['pack']=='')) {
		...
		if (!can_user_order($b_row, $_SESSION['MDS_ID'], $_REQUEST['pack'])) { // one more check before continue
			...
			$BTCpayment = $pack['price'] * $block_count;
            $DogePayment = $order_row['price'];
	echo "Total Cost will be $DogePayment";
    $token = ($order_id+3)  *76745476;
				?>

				<input type='button' class='big_button' value="<?php echo htmlentities($label['advertiser_o_confpay_button']); ?>" Onclick="window.location='checkout.php?action=confirm&token=<?php echo "$token"?>&BID=<?php echo $BID; ?>'">
